...
function* fetchDataSaga(action) {
  ...
  const { data, hasMoreData } = yield call(effects.fetchData, {
  ...
  yield put({
    ...
    payload: {
      data: data.length ? oldData.concat(data) : oldData,
      hasMoreData
    }
  });
...
const sagas = [takeLatest(actions.FETCH_DATA, fetchDataSaga)];

export { sagas as default, fetchData, fetchDataSaga };
